...
void __accmut__filter__variant(int from, int to) {
    ...
    //printf("FILTER VARIANTS END, RESULT IS:\n");
    //for(i = 0; i < recent_num; ++i) {
        //printf("%d ", recent_set[i]);
    //}
    //printf("\n");
}
//
...
int __accmut__process_i32_arith(int from, int to, int left, int right){
    //printf("CAL %d %d %d %d %d EX\n", ALLMUTS[to]->s_pre, to, left, right, MUTATION_ID);
...
int __accmut__cal_i32_bool(int pre, int a, int b){
	switch(pre){
		...
		case 41: return a <= b;
		default:
			printf("ERROR : __accmut_cal_i32_bool with %d !!!\n", pre);
			exit(0);
	}
}
